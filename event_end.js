...
function off(ele,eventType,fn){ //负责移除
    ...
    //forIE6-8
    var a = ele['myEvent'+eventType];   //oDiv.myEvnethaha;  undefined
    if(a){
...
function run(e){ //负责找到数组并且按照顺序执行
    // this就是ele。this是在on方法中就已经通过call方式处理好了
    // ele['myEvent'+eventType]
    e =  window.event;
    var a = this['myEvent'+ e.type]; //
    if(a){ //这个条件成立那么必定是ie6-8浏览器,在标准浏览器根本就不用添加这个自定义属性并且还赋值数组
